...
class SkillManager:
    ...
    def list_skill_files(self, skill: SkillInfo) -> tuple[list[str], list[str]]:
        ...
        # Calculate length for fast string slicing of the base path (+1 for separator)
        base_dir_len = len(base_dir_str)
        if not base_dir_str.endswith(os.sep):
            base_dir_len += 1

        # Bolt: Avoid heavy pathlib object instantiation by using os.walk
        # This is ~90% faster for deep directories
                ...
                for root, _, files in os.walk(str(resource_dir)):
                    for file_name in files:
                        if not file_name.startswith("."):
                            ...
                            rel_path = full_path[base_dir_len:].replace(os.sep, "/")
