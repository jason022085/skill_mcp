...
class SkillScanner:
    ...
    def _is_excluded(self, path: Path) -> bool:
        ...
        # Check for excluded directories in path using set intersection (O(N))
        if not self.EXCLUDED_DIRS.isdisjoint(parts):
            return True

        # Skip hidden files/directories
        # Note: Using a basic for loop here is demonstrably faster in CPython
        # than any() with a generator expression due to lack of generator overhead.
        for part in parts:  # noqa: SIM110
