...
class SkillScanner:
    ...
    def _is_excluded(self, path: Path) -> bool:
        ...
        """
        # Fast subset check using set intersection for O(1) performance
        # Much faster than substring checks on str(path)
        if not self.EXCLUDED_DIRS.isdisjoint(path.parts):
            return True

        # Skip hidden files/directories
        # Using any() generator expression is typically faster than explicit loop
